...
async function main() {
  ...
  const counts = await client.unsafe(`
    SELECT
      ...
      (SELECT COUNT(*) FROM public.lookups) as lookups_count,
      (SELECT COUNT(*) FROM public.cases_imported) as cases_imported_count,
      (SELECT COUNT(*) FROM public.cases_user) as cases_user_count,
